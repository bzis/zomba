...
angular.module('security.wsse', ['cookiesModule', 'ngResource']).
...
factory('TokenHandler', ['$http', '$window', 'Cookies', 'Base64', function($http, $window, $cookieStore, Base64) {
    var tokenHandler = {};
    var token = 'none';
    ...
    tokenHandler.getCredentials = function (username, secret) {
        ...
        if (angular.isDefined($window.crypto) && angular.isDefined($window.crypto.getRandomValues)) {
            var array = new Uint32Array(1);
            $window.crypto.getRandomValues(array);
            seed = array[0] + '';
        } else {
            seed = Math.floor(Math.random() * (1000000 - 1 + 1)) + 1 + '';
        }
        ...
        // console.log(username, digest, nonce, created);
